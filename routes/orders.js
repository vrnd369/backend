...
router.post('/refresh-tracking/:orderId', authenticateToken, async (req, res) => {
  try {
    ...
    const trackingUpdates = await shiprocket.autoUpdateTrackingDetails(order.shiprocketShipmentId);
    ...
    if (trackingUpdates) {
      ...
      if (trackingUpdates.trackingNumber && trackingUpdates.trackingNumber !== order.trackingNumber) {
        ...
        updates.trackingNumber = trackingUpdates.trackingNumber;
        updated = true;
      ...
      if (trackingUpdates.courierName && trackingUpdates.courierName !== order.courierName) {
        ...
        updates.courierName = trackingUpdates.courierName;
        updated = true;
      ...
      if (trackingUpdates.trackingUrl && trackingUpdates.trackingUrl !== order.trackingUrl) {
        ...
        updates.trackingUrl = trackingUpdates.trackingUrl;
        updated = true;
      ...
      if (updated) {
        await order.save();
        console.log('✅ Order updated with refreshed tracking details');
      }
      
      res.json({
        status: 'success',
        message: updated ? 'Tracking details refreshed successfully' : 'No new updates available',
        updated: updated,
        updates: updated ? updates : null,
        order: {
          orderId: order.orderId,
