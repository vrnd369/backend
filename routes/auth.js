...
// Get current user profile (protected - alternative endpoint)
router.get('/profile', authenticateToken, async (req, res) => {
  try {
...
// Update current user profile (protected - no userId needed)
router.put('/profile', authenticateToken, async (req, res) => {
  const { firstName, lastName, phone, profilePic, shippingAddress, billingAddress } = req.body;
  ...
  try {
    ...
    res.status(200).json({ status: 'success', message: 'Profile updated successfully', user: updated });
  } catch (err) {
    console.error('Profile update error:', err);
